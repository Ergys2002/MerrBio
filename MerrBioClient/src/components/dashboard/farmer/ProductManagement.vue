...
const validateForm = (): boolean => {
    ...
    if (productForm.minAvailableQuantity <= 0) {
        formErrors.minAvailableQuantity = t(
            "validation.quantityGreaterThanZero"
        );
        isValid = false;
    }
...
const submitProductForm = async () => {
    ...
    try {
        if (isEditing.value && productForm.id) {
            ...
            formData.append(
                "minAvailableQuantity",
                productForm.minAvailableQuantity.toString()
            );
            formData.append(
                "maxAvailableQuantity",
                productForm.maxAvailableQuantity.toString()
            );
            formData.append(
                "minimumOrderQuantity",
                productForm.minimumOrderQuantity.toString()
            );
            ...
            if (productForm.thumbnail) {
                ...
                alert(
                    t("validation.thumbnailRequired") ||
                        "Please select a thumbnail image for the product"
                );
                return;
            }
        ...
        try {
            await productStore.fetchProducts(token);
...
/**
 * Format thumbnail URL to include the API base path
 * @param url - The thumbnail URL from the backend
 * @returns Formatted URL for displaying the image
 */
const formatImageUrl = (url: string | null): string => {
    if (!url) return "";

    // If URL already contains the full path, return it as is
...
/**
 * Retry fetching products, called from error state button
 */
async function retryFetchProducts() {
    ...
    try {
        ...
        loadingError.value =
            productStore.error || t("errors.fetchProductsFailed");
    } finally {
        loadingInitial.value = false;
    }
}
...
onMounted(async () => {
    ...
    try {
        ...
        // Fetch all products using the general products endpoint
        await productStore.fetchProducts(token);
            ...
            <div class="modal-content">
                ...
                <form @submit.prevent="submitProductForm" class="product-form">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input
                                ...
                                min="0"
                                v-model.number="
                                    productForm.minAvailableQuantity
                                "
                                :class="{
                                    error: formErrors.minAvailableQuantity,
                                }"
                            />
                            <span
                                v-if="formErrors.minAvailableQuantity"
                                class="error-message"
                            >
                        ...
                        <div class="form-group">
                            ...
                            <input
                                ...
                                min="0"
                                v-model.number="
                                    productForm.maxAvailableQuantity
                                "
                            />
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        />
                        <small
                            >Main image that will be displayed for this
                            product</small
                        >
                    </div>
                ...
                <tbody>
                    <tr v-for="product in filteredProducts" :key="product.id">
                        <td class="product-info">
                            ...
                            >
                                <img
                                    :src="formatImageUrl(product.thumbnailUrl)"
                                    :alt="product.name"
                                />
                            </div>
